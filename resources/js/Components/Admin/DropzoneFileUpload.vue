<template>
    <div
        class="mb-3 dropzone dz-clickable"
        ...
        initDropzone() {
            ...
            this.dropzone = new Dropzone(this.$el, {
                ...
                removedfile: (file) => {
                    ...
                    if (typeof file.xhr !== 'undefined') {
                        let response = JSON.parse(file.xhr.response);
                        imageId = response.product_image_id;
                    } else {
                    ...
                    this.$swal({
                        ...
                        if (result.isConfirmed) {
                            ...
                            axios
                                .delete
                                // route('admin.product-images.destroy', {product: this.productId, product_image: imageId}),
                                ()
                                .then((res) => {
                                    console.log(res);
                                    // 表示されている画像を消去
...
.dropzone .btn-danger,
.dropzone .btn-danger:hover {
    color: #fff;
    background-color: #f5365c;
    border-color: #f5365c;
}
.dropzone .btn-group-sm > .btn,
.dropzone .btn-sm {
