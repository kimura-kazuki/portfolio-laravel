...
<template>
    ...
    <AppLayout>
        ...
        <JetBarContainer>
            ...
            <Link :href="route('admin.users.create')" class="inline-flex items-center px-4 py-2 mb-3 text-sm font-semibold tracking-widest text-white uppercase transition bg-blue-800 border border-transparent rounded-md shadow hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring focus:ring-blue-300 disabled:opacity-25" id="create"> 新規追加 </Link>
            ...
            >
                <template #cell(id)="{ item: user }">
                    ...
                    <div class="text-xs text-gray-500">{{ user.created_at_formated }}</div>
                </template>
                ...
                <template #cell(role_label)="{ item: user }">
                    <JetBarBadge :text="user.role_label" :type="roleColor[user.role_name]" />
                </template>
                <template #cell(is_approved_label)="{ item: user }">
                    <JetBarBadge :text="user.is_approved_label" :type="statusType[user.is_approved_name]" />
                </template>
                <template #cell(参照)="{ item: user }">
                    <div class="relative flex-grow">
                        <Link :href="route('admin.users.show', user.id)" class="text-indigo-600 hover:text-indigo-900">参照</Link>
                    </div>
                </template>
                <template #cell(編集)="{ item: user }">
                    <div class="relative flex-grow">
                        <Link :href="route('admin.users.edit', user.id)" class="text-indigo-600 hover:text-indigo-900">編集</Link>
                    </div>
                </template>
                <template #cell(削除)="{ item: user }">
                    <div
                        ...
                        <button class="text-gray-400 hover:text-gray-500" @click="deleteUser(user.id, index)">
